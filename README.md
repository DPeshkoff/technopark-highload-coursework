...
> 408,8 / 30 = 13,63 миллионов "*сообщений*"

Каждую секунду пользователи добавляют 

> 13'630'000 / (24 **⋅** 60 **⋅** 60) = 157,75 "*сообщений*"

следовательно, с учетом запросов на получение формы и отправку, 

> 157,75 **⋅** 2 = 315,5 RPS.

В среднем один пользователь проводит во ВКонтакте **37 минут в день**, по данным Mediascope (*webindex, ноябрь 2019, Россия, 0+*). Пользователи десктопной версии тратят на соцсеть меньше времени — в среднем **20 минут в день**. Продолжительность сессий на мобильных устройствах выше и составляет **39 минут в день**[<sup>8</sup>](#source-8).

В среднем число активных пользователей в час не превышает **4,4** %[<sup>8</sup>](#source-8):

<img src="./images/hourly-vk.png">

Следовательно, каждый час активны примерно:

> 96'140'000 / 24 = 4'005'833,3 пользователей

Расчитаем число запросов на получение "*сообщений*" в секунду (допустим, что пользователи читают по 0,5 "*сообщения*" в секунду):

> 0,5 **⋅** 4'005'834= 2'002'917 RPS.

Расчитаем число запросов на лайки в секунду:

> 1'000'000'000 / (24 **⋅** 60 **⋅** 60) = 11'574,07 RPS

Расчитаем число личных сообщений в секунду:

> (15'000'000'000 - 13'630'000) / (24 **⋅** 60 **⋅** 60) = 173'453,36 RPS

### Среднее количество RPS по типам действий

| Действие | RPS | Взаимодействие
|:---:|:----------- | :----------:|
| Добавление сообщения | 315,5      | запись |
| Чтение сообщений | 2'002'917     |  чтение |
| Оценки | 11'574,07  |  запись |
| Личные сообщения | 173'453,36   |  запись |
| Личные сообщения | 173'453,36 | чтение |

В сумме:

**185 345,93 RPS на запись**

**2 176 370,36 RPS на чтение**
...
8. <a id="source-8"></a> [20 цифр и фактов о ВКонтакте, необходимые маркетологу в 2020](https://popsters.ru/blog/post/svezhie-dannye-o-vk)
